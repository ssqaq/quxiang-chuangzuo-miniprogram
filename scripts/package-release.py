...
def main() -> None:
    ...
    manifest = "\n".join(
        [
            ...
            "数据库初始化：部署 api 后执行 scripts/init-cloud-database.ps1，自动补齐 18 个集合（含 watermark_transfer_temp_assets）",
            "数据库索引：执行 scripts/check-cloud-database-indexes.ps1，先检查再逐项确认创建 13 组必需索引",

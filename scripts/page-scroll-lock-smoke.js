...
assert.ok(
  /page-meta[^>]*page-style="\{\{pageScrollStyle\}\}"/.test(wxml),
...
assert.ok(
  (wxml.match(/capture-catchtouchmove="onCanvasTouchMove"/g) || []).length >= 1,
  "画布视口必须在捕获阶段拦截触摸移动"
);
assert.ok(
...
assert.ok(
  /pageScrollStyle:\s*""/.test(js)
    && /position: fixed/.test(js)
    && /top: -\$\{scrollTop\}px/.test(js),
  "页面锁定必须使用固定定位保留当前滚动位置"
);
assert.ok(

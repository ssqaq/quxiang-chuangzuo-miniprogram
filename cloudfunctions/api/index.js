const API_BUILD_VERSION = "0.57.32";
const API_BUILD_MARKER = "API_BUILD_TAG_AUTO_VERSION_V05732";
...
function resolveImageBackupConfig(overrides = {}) {
  ...
  return {
    ...
    providerKey: normalizeProviderKey(
      overrideString(image, "providerKey", providerBuiltinKey(image && image.provider) || providerStableKey(image && image.provider || "lingyun")),
      providerBuiltinKey(image && image.provider) || providerStableKey(image && image.provider || "lingyun")
    ),
    baseUrl: overrideString(image, "baseUrl", firstEnv(

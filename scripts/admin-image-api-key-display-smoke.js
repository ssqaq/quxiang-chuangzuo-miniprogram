...
async function verifyAdminPage() {
  ...
  assert.strictEqual(
    Object.prototype.hasOwnProperty.call(savePayloads[3], "image"),
    false,
    "只改备用模型参数时不应重复提交主模型段"
  );
}
...
function verifyMarkupAndStaticBoundaries() {
  ...
  ["image", "imageBackup"].forEach((section) => {
    ...
    assert.strictEqual(/\bdisabled\b/.test(inputs[section]), false);
  });
  ["face", "faceBackup", "analysis", "analysisBackup"].forEach((section) => {
    assert.ok(inputs[section]);
    assert.strictEqual(/\bpassword\b/.test(inputs[section]), true);
  });
  ["video", "videoBackup"].forEach((section) => {
    assert.ok(inputs[section]);

const API_BUILD_VERSION = "0.43.0";
const API_BUILD_MARKER = "API_BUILD_TAG_AUTO_VERSION_V0430";
...
async function getModelUsageStats(event, context) {
  ...
  const includeDetails = Boolean(event && event.includeDetails);
  ...
  try {
    ...
    const responseStats = Object.assign({}, stats);
    if (!includeDetails) delete responseStats.details;
    return jsonResponse(true, Object.assign(responseStats, {
    ...
    const emptyStats = aggregateModelUsageEvents([], days);
    if (!includeDetails) delete emptyStats.details;
    return jsonResponse(true, Object.assign(
      emptyStats,
      {
        eventCount: 0,
...
async function exportModelUsageStats(event, context) {
  if (!isAdminContext(context)) return adminForbidden();
  const days = Math.max(1, Math.min(90, Number(event && event.days) || 30));
  const stats = await getModelUsageStats({ days, includeDetails: true }, context);

...
assert.strictEqual(dangerousReport.probe.buildVersion, "0.24.0");
...
function event(dayOffset, failureType, index = 0) {
  ...
  return Object.assign(
    ...
    test.normalizeAutoFaceFailureReport({
      ...
      probe: {
        status: probeStatus,
        buildVersion: probeStatus === "ok" ? "0.24.0" : "0.21.11",
...
assert.ok(stats.recent.some((item) => item.probe && item.probe.buildVersion === "0.24.0"));

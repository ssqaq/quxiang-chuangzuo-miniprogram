...
const dangerousReport = test.normalizeAutoFaceFailureReport({
  ...
  probe: {
    ...
    requestId: "probe-1",
    buildVersion: "0.28.3",
    buildMarker: "API_BUILD_TAG_20260824_BACKGROUND_REFERENCE_V283",
...
assert.strictEqual(dangerousReport.probe.buildVersion, "0.28.3");
...
function event(dayOffset, failureType, index = 0) {
  ...
  return Object.assign(
    ...
    test.normalizeAutoFaceFailureReport({
      ...
      probe: {
        status: probeStatus,
        buildVersion: probeStatus === "ok" ? "0.28.3" : "0.21.11",
...
assert.ok(stats.recent.some((item) => item.probe && item.probe.buildVersion === "0.28.3"));

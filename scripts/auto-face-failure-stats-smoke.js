...
const dangerousReport = test.normalizeAutoFaceFailureReport({
  ...
  probe: {
    ...
    requestId: "probe-1",
    buildVersion: "0.26.2",
    buildMarker: "API_BUILD_TAG_20260824_POINTS_RESET_V262",
...
assert.strictEqual(dangerousReport.probe.buildVersion, "0.26.2");
...
function event(dayOffset, failureType, index = 0) {
  ...
  return Object.assign(
    ...
    test.normalizeAutoFaceFailureReport({
      ...
      probe: {
        status: probeStatus,
        buildVersion: probeStatus === "ok" ? "0.26.2" : "0.21.11",
        buildMarker: probeStatus === "ok"
          ? "API_BUILD_TAG_20260824_POINTS_RESET_V262"
...
assert.ok(stats.recent.some((item) => item.probe && item.probe.buildVersion === "0.26.2"));

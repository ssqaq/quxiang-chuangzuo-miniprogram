...
const dangerousReport = test.normalizeAutoFaceFailureReport({
  ...
  probe: {
    ...
    requestId: "probe-1",
    buildVersion: "0.24.3",
    buildMarker: "API_BUILD_TAG_20260824_CHECKIN_STRIP_ID_V243",
...
assert.strictEqual(dangerousReport.probe.buildVersion, "0.24.3");
...
function event(dayOffset, failureType, index = 0) {
  ...
  return Object.assign(
    ...
    test.normalizeAutoFaceFailureReport({
      ...
      probe: {
        status: probeStatus,
        buildVersion: probeStatus === "ok" ? "0.24.3" : "0.21.11",
...
assert.ok(stats.recent.some((item) => item.probe && item.probe.buildVersion === "0.24.3"));

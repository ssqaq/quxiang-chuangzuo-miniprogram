const API_BUILD_VERSION = "0.57.77";
const API_BUILD_MARKER = "API_BUILD_TAG_AUTO_VERSION_V05777";
...
async function resolveEffectiveConfigs(options = {}) {
  ...
  const faceBackup = resolveFaceBackupConfig(projected.faceBackup);
  const analysisBackup = resolveAnalysisBackupConfig(projected.analysisBackup);
  const videoBackup = resolveVideoBackupConfig(projected.videoBackup);
  return {
    runtime: projected || {
...
async function getPaymentMonitor(context) {
  if (!isAdminContext(context)) return adminForbidden();
  let snapshot = null;
  try {
    snapshot = await paymentMonitor.readSnapshot(db);
  } catch (_error) {
    snapshot = null;
  }
  return jsonResponse(true, {
...
async function getAdminImageApiKeys(context) {
  ...
  return jsonResponse(true, {
    ...
    analysisBackup: {
      apiKey: String(configs.analysisBackup && configs.analysisBackup.apiKey || "")
    },
    image: {
    ...
    videoBackup: {
      apiKey: String(configs.videoBackup && configs.videoBackup.apiKey || "")
    }
  });
}
...
exports.main = async (event = {}, context) => {
  ...
  try {
    ...
    else if (action === "getPaymentMonitor") result = await getPaymentMonitor(context);

...
const dangerousReport = test.normalizeAutoFaceFailureReport({
  ...
  probe: {
    ...
    requestId: "probe-1",
    buildVersion: "0.27.0",
    buildMarker: "API_BUILD_TAG_20260824_VIDEO_USER_EXPORT_V270",
...
assert.strictEqual(dangerousReport.probe.buildVersion, "0.27.0");
...
function event(dayOffset, failureType, index = 0) {
  ...
  return Object.assign(
    ...
    test.normalizeAutoFaceFailureReport({
      ...
      probe: {
        status: probeStatus,
        buildVersion: probeStatus === "ok" ? "0.27.0" : "0.21.11",
        buildMarker: probeStatus === "ok"
          ? "API_BUILD_TAG_20260824_VIDEO_USER_EXPORT_V270"
...
assert.ok(stats.recent.some((item) => item.probe && item.probe.buildVersion === "0.27.0"));

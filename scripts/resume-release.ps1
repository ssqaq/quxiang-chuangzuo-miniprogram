...
$contextTransport = ""
if ($context.PSObject.Properties["gitTransport"] -and $null -ne $context.gitTransport) {
    $contextTransport = [string]$context.gitTransport.transport
}
if (-not $PSBoundParameters.ContainsKey("GitTransport") -and $contextTransport -eq "Ssh443") {
    if ([string]::IsNullOrWhiteSpace($SshKeyPath)) { throw "Context requires Ssh443 resume; pass -SshKeyPath explicitly." }
    Set-ReleaseGitTransport -Transport Ssh443 -SshKeyPath $SshKeyPath
}
$context | Add-Member -NotePropertyName gitTransport -NotePropertyValue $script:GitTransportState -Force
...
function Ensure-ResumeReleaseWorktree {
    ...
    $releaseGateRepairOnly = $dirtyLines.Count -eq 1 -and [string]$dirtyLines[0] -match '^\s*M\s+scripts/release-gate\.ps1\s*$'
    if ($releaseGateRepairOnly) {
        $canonicalGate = Join-Path $canonicalRepo "scripts/release-gate.ps1"
        $worktreeGate = Join-Path $resolved "scripts/release-gate.ps1"
        if (-not (Test-Path -LiteralPath $canonicalGate -PathType Leaf) -or
            -not (Test-Path -LiteralPath $worktreeGate -PathType Leaf) -or
            (Get-FileHash -LiteralPath $canonicalGate -Algorithm SHA256).Hash -ne (Get-FileHash -LiteralPath $worktreeGate -Algorithm SHA256).Hash) {
            throw "恢复工作树的 release-gate 修复与 canonical 不一致，拒绝部署：$resolved"
        }
    }
    if ($dirtyLines.Count -gt 0 -and -not $previewOnlyDirty -and -not $releaseGateRepairOnly) { throw "恢复工作树不是干净状态，拒绝部署：$resolved" }
    ...
    # A source-tooling release carries the validator that produced its
    # immutable source fingerprint. Reuse that exact validator during resume;
    # falling back to canonical tooling is safe only for older contexts.
    $packageScript = Join-Path $resolved "scripts/package-release.py"
    ...
    $probeOutput = @(& python $packageScript --check-only --release-context $contextPath 2>&1 | ForEach-Object { [string]$_ })

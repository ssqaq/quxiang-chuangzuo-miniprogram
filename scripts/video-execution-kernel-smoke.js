...
async function main() {
...

  {
    ...
    const configs = {
      video: {
        configured: true,
        provider: "primary-video",
        providerKey: "primary-video-key",
      ...
      videoBackup: {
        enabled: true,
        configured: true,
        provider: "backup-video",
        providerKey: "backup-video-key",
    ...
    harness.services.config.resolve = async () => configs;
    harness.services.provider.buildPayload = (payload, buffer, video) => ({
      model: video.model,
      prompt: payload.prompt,
      resolution: video.resolution,
      duration: payload.durationSeconds || 3
    });
    harness.services.provider.create = async ({ video }) => {
      createProviders.push(video.providerKey);
      if (video.providerKey === "primary-video-key") {
        const error = new Error("主视频服务暂时不可用");
        error.status = 503;
        error.code = "VIDEO_PRIMARY_UNAVAILABLE";
        throw error;
      }
      return {
        taskId: "backup-video-task",
        status: "processing",
        providerStatus: "queued"
      };
    };
    harness.kernel = createVideoExecutionKernel(harness.services);
    const created = await createTask(harness, "video-backup-request");
    assert.strictEqual(created.ok, true);
    assert.strictEqual(created.provider, "backup-video");
    assert.strictEqual(created.providerKey, "backup-video-key");
    assert.deepStrictEqual(
      createProviders,
      ["primary-video-key", "backup-video-key"],
      "主视频创建遇到 5xx 后必须只切换一次备用服务"
    );
    assert.strictEqual(harness.state.operation.providerRole, "backup");
    assert.strictEqual(harness.state.operation.providerKey, "backup-video-key");

    // 服务商外部 ID 改名后，历史任务仍应按稳定 providerKey 查询原服务。
    configs.videoBackup = Object.assign({}, configs.videoBackup, {
      provider: "backup-video-renamed"
    });
    harness.services.provider.query = async ({ video }) => {
      assert.strictEqual(video.providerKey, "backup-video-key");
      return {
        status: "processing",
        providerStatus: "running",
        videoURL: "",
    ...
    const queried = await harness.kernel.queryVideoTask({
      requestId: "video-backup-request",
      taskId: "backup-video-task"
    }, { OPENID: "video-user" });
    assert.strictEqual(queried.ok, true);
    assert.strictEqual(queried.provider, "backup-video-renamed");
  }

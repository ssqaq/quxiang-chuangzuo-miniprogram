...
function testPageMarkup() {
  ...
  assert.strictEqual((wxml.match(/返回工作台/g) || []).length, 1);
  ...
  assert.ok(wxss.includes(".back-button"));
  assert.ok(wxss.includes("gap: 16rpx"));
  assert.ok(wxss.includes("height: 88rpx"));
  assert.ok(wxss.includes("font-size: 26rpx"));

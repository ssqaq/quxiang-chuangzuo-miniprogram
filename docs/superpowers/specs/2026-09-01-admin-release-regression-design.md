...
15. 严格截图工作流运行在带微信开发者工具的 Windows self-hosted runner；截图失败不得降级复用旧证据。
16. 视觉状态固定为默认、展开、备用关闭、视频模式；设备固定为 375x812、390x844、430x932，全部校验截图尺寸、横向溢出和 SHA256。
17. 版本归档生成 `index.json` 与自包含 `index.html`；生成前必须重验归档文件字节数和 SHA256。
